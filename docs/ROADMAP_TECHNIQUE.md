...
7. **Pastille synchro** : appui long sur erreur → `openTechnicalFeedback` (`SyncStatusPill`).
8. **Documentation** : ce fichier, `ARCHITECTURE_RUNTIME.md`, modèle de rapport perf, checklist install APK dans le template.
...
| P4 | Étendre `openTechnicalFeedback` aux autres erreurs **catch** métier (ex. écrans gérant / livreur) | Panier client : bouton « Détail technique » si échec envoi (`pushFailed`) — voir `app/client/panier.tsx` |
