/**
 * Sondes debug plan Husko : console en __DEV__ + envoi optionnel vers ingest local.
 */
const INGEST =
...
export function emitDebugProbe(p: ProbePayload) {
  const line = { sessionId: SESSION, timestamp: Date.now(), location: "debugProbe", ...p };
  if (__DEV__) console.log("[HuskoDebug]", JSON.stringify(line));
  postIngest(line);
}
...
export function emitBootDebugProbes(args: {
  ...
  emitDebugProbe({
    ...
    message: "Flags Maps",
    data: { mapsAndroidKeyOk: args.mapsAndroidKeyOk, mapsIosKeyOk: args.mapsIosKeyOk },
    runId: "boot",
  });
  emitDebugProbe({
    hypothesisId: "H3_app_variant",
    message: "Variante app",
