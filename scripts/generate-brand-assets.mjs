...
/** Ciel + horizon — brique, orange smog, violet nuit (rappel GTA / Venice sunset). */
...
<svg width="${s}" height="${s}" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${s} ${s}">
  <defs>
    <linearGradient id="sky" x1="0%" y1="0%" x2="0%" y2="100%">
      ...
      <stop offset="28%" stop-color="#7c2d12"/>
      <stop offset="52%" stop-color="#ea580c"/>
      <stop offset="78%" stop-color="#9a3412"/>
      <stop offset="100%" stop-color="#0c0305"/>
    </linearGradient>
    <linearGradient id="chrome" x1="0%" y1="0%" x2="0%" y2="100%">
      <stop offset="0%" stop-color="#fef9c3"/>
      <stop offset="35%" stop-color="#ff5a38"/>
  ...
  <line x1="18%" y1="${Math.round(s * 0.67)}" x2="82%" y2="${Math.round(s * 0.67)}" stroke="#22d3ee" stroke-opacity="0.35" stroke-width="${Math.max(2, Math.round(s * 0.004))}"/>
...
<svg width="${w}" height="${h}" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${w} ${h}">
  <defs>
    <linearGradient id="bhSky" x1="0%" y1="0%" x2="0%" y2="100%">
      ...
      <stop offset="82%" stop-color="#431407"/>
      <stop offset="100%" stop-color="#0c0305"/>
    </linearGradient>
  ...
  <circle cx="${sunCx}" cy="${sunCy}" r="${sunR}" fill="url(#bhSun)" opacity="0.55" filter="url(#bhGlow)"/>
  <rect x="0" y="${Math.round(h * 0.62)}" width="${w}" height="${Math.round(h * 0.38)}" fill="#050203" opacity="0.55"/>
</svg>`;
}
